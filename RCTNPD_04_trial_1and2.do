...
********************************************************************************
merge 		m:1 studyid using "${datadir}\all\attributedataset_randomised.dta", update keepusing(byr bayley* parity smokdur died matage iq_score *iq* matedu alcdur apgar5m kps_fullscore teen centre gestage bwt multiple* address_tot age_firstadd age_lastaddgrp age_lastadd fup*)
replace		unmatched =1 if _merge==2
drop		  _merge
********************************************************************************
...
/*==============================================================================*/
cap drop 	dup byr
save		"${datadir}\04-ev_linkage_t1.dta", replace	
log 		close
